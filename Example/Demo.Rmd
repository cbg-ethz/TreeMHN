...
g1 <- ggpubr::annotate_figure(plot_Theta(true_Theta, to_show = idx), top = "True MHN")
g2 <- ggpubr::annotate_figure(plot_Theta(pred_Theta, to_show = idx), top = "TreeMHN")
g3 <- ggpubr::annotate_figure(plot_Theta(pred_Theta_w_SS, to_show = idx), top = "TreeMHN with stability selection")
```
...
To use another dataset, please make sure it is in dataframe format with four columns:
...
To ensure enough precision, we run stability selection with $\gamma = 0.1$ and a threshold of $99\%$ and obtain a vector of non-selected elements over $1000$ subsamples. Again, we recommend to run the code using the `parallel` package on a cluster.
...
Then we refit the model with $\gamma = 0$ by masking the non-selected elements.
...
Given the estimated MHN, we can plot the most probable evolutionary trajectories of a given length using the `plot_pathways` function:

```{r plot_AML_pathways, fig.height=6, fig.width=17}
