...
To ensure enough precision, we run stability selection with $\gamma = 0.05$ and a threshold of $99\%$ and obtain a vector of non-selected elements over $1000$ subsamples. Again, we recommend to run the code using the `parallel` package on a cluster.
...
The diagonal entries represent the baseline rates of mutations to occur, independent of other events:
...
Given the estimated MHN, we can plot the most probable evolutionary trajectories of a given length using the `plot_pathways` function. Here we choose length $4$.

```{r, fig.height=6, fig.width=17}
plot_pathways(AML_Theta, mutations = AML$mutations, n_order = 4, top_M = 10)
```

Given a particular tree and the estimated MHN, we can also find the next most probable mutational events using the `next_mutation` function.
