...
##' @author Xiang Ge Luo
##' @export
input_tree_df <- function(n, tree_df, mutations = NULL, tree_labels = NULL) {

  # Check input format
  if (length(setdiff(colnames(tree_df), c("Tree_ID","Node_ID","Mutation_ID","Parent_ID"))) != 0) {
...
output_tree_df <- function(tree_obj) {
  ...
  tree_df <- data.frame(matrix(ncol = 4, nrow = 0))

  for (tree in tree_obj$trees) {
    tree_df <- rbind(tree_df, output_one_tree_df(tree))
  }

  colnames(tree_df) <- c("Tree_ID","Node_ID","Mutation_ID","Parent_ID")
...
output_one_tree_df <- function(tree) {
  ...
  temp_df <- cbind(tree_ID_temp, node_ID_temp, mutation_ID_temp, parent_ID_temp)
  colnames(temp_df) <- c("Tree_ID","Node_ID","Mutation_ID","Parent_ID")
